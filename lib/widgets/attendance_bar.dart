...
class attendance_percent_diagram extends StatefulWidget {
  final double rad;
  const attendance_percent_diagram({super.key, required this.rad});
...
class _attendance_percent_diagramState
  ...
  void Percentage() async {
    var list = await getAggregateAttendance();
    var i = 0;
    for (final MapEntry in list.entries) {
      percentage = percentage + MapEntry.value;
      i += 1;
    }
    percentage /= i;
  }
  ...
  Widget build(BuildContext context) {
    return CircularPercentIndicator(
      ...
      lineWidth: 13.0,
      animation: true,
      percent: double.parse(percentage.toStringAsFixed(2)),
      center: new Text(
        // ignore: prefer_interpolation_to_compose_strings
        (double.parse(percentage.toStringAsFixed(2)) * 100).toString() + "%",
